...
pub extern "C" fn get_musicians_json(db_path: *const c_char) -> *mut c_char {
    ...
    match RadioGolhaCore::open(&path) {
        Ok(core) => {
            let conn = core.connection();
            let mut stmt = conn.prepare("SELECT a.id, a.name, a.avatar, COUNT(DISTINCT pp.program_id) FROM program_performers pp JOIN performer p ON p.id = pp.performer_id JOIN artist a ON a.id = p.artist_id GROUP BY p.id ORDER BY a.name ASC").unwrap();
            let rows: Vec<_> = stmt.query_map([], |row| {
                Ok(json!({ "id": row.get::<_, i64>(0)?, "name": row.get::<_, String>(1)?, "avatar": row.get::<_, Option<String>>(2)?, "instrument": "نوازنده", "programCount": row.get::<_, i64>(3)? }))
            }).unwrap().filter_map(|r| r.ok()).collect();
            rust_str_to_c(to_string(&rows).unwrap())
        },
        Err(_) => rust_str_to_c("[]".to_string())
