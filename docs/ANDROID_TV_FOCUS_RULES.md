...
- Loading and empty states must still expose a real focus anchor when another section points to them.
- Sidebar items must explicitly wire up/down to adjacent sidebar items; do not let Compose infer vertical focus across the spacer before settings/help.
...
- Never point `up`, `down`, `left`, or `right` to a `FocusRequester` whose target is conditionally not rendered.
